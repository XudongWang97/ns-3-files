...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("LimitedTransmit", "Enable limited transmit",
                   ...
                   MakeBooleanChecker ())
    // DCTCP
    .AddAttribute ("UseDCTCP", "Enable sending and receiving as described in DCTCP",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketBase::m_useDCTCP),
                   MakeBooleanChecker ())
    .AddTraceSource ("RTO",
...
TcpSocketState::TcpSocketState (const TcpSocketState &other)
  : Object (other),
    ...
    m_g (other.m_g),
    m_markedAckCount (other.m_markedAckCount),
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if (m_useDCTCP)
    {
      if (!m_CEstate && (header.EcnTypeToString (header.GetEcn ()) == "CE"))
        {
          m_CEstate = true;
          SendEmptyPacket (TcpHeader::ACK);
        }
      else if (m_CEstate && (header.EcnTypeToString (header.GetEcn ()) != "CE"))
        {
          m_CEstate = false;
...
TcpSocketBase::EnterRecovery ()
{
  ...
  if (!m_useDCTCP)
  ...
  else
    {
      if (m_sackEnabled)
        {
          m_tcb->m_cWnd = (1.0 - m_tcb->m_alpha / 2.0) * m_tcb->m_cWnd;
        }
      else
        {
          m_tcb->m_cWnd = (1.0 - m_tcb->m_alpha / 2.0) * m_tcb->m_cWnd + m_dupAckCount * m_tcb->m_segmentSize;
        }
    }
  /*
  if (m_sackEnabled)
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if (GetIpTos () || m_useDCTCP)
    {
      ...
      if (!m_useDCTCP)
        ...
        {
          ipTosTag.SetTos (GetIpTos () | Ipv4Header::ECN_ECT0);
        }
  ...
  if (!m_useDCTCP)
    {
      isAck = flags == TcpHeader::ACK;
    }
  else
    {
      isAck = (flags == TcpHeader::ACK) || (flags == (TcpHeader::ACK | TcpHeader::ECE));
    }
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
  ...
  // DCTCP
  if (m_useDCTCP && m_tcb->m_ackCount)
    {
      m_tcb->m_alpha = (1.0 - m_tcb->m_g) * m_tcb->m_alpha + 
        1.0 * m_tcb->m_g * m_tcb->m_markedAckCount / m_tcb->m_ackCount;
      m_tcb->m_markedAckCount = 0;
      m_tcb->m_ackCount = 0;
    }
