...
TcpHybla::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpHybla")
    ...
    .AddAttribute ("RRTT", "Reference RTT",
                   ...
                   MakeTimeChecker ())
    // MYTCP
    .AddAttribute ("useMYTCP", "flag to use MYTCP",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpHybla::m_useMYTCP),
                   MakeBooleanChecker ())
    .AddAttribute ("mulThresh", "Multiple Threshold",
                   DoubleValue (2),
                   MakeDoubleAccessor (&TcpHybla::m_mulThresh),
                     ...
                     const Time &rtt)
{
  ...
  if (!m_useMYTCP)
    ...
    {
      if (rtt < m_minRtt / sqrt (m_mulThresh))
        {
          RecalcParam (tcb, rtt);
          m_minRtt = rtt;
          NS_LOG_DEBUG ("Updated m_minRtt=" << m_minRtt);
